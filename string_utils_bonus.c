...
void	ft_putstr_fd_bonus(char *str, int fd)
{
	...
	{
		write(fd, str, 1);
		ft_putstr_fd_bonus(str + 1, fd);
	}
}

static void	check_overflow_bonus(long digit, long result, long sign)
{
...
static int	parse_sign_and_skip_spaces_bonus(char **s)
{
	int	sign;
...
double	ft_atodbl_bonus(char *s)
{
	...
	fractional_part = 0;
	pow = 1;
	sign = parse_sign_and_skip_spaces_bonus(&s);
	while (*s != '.' && *s)
	{
		check_overflow_bonus(*s - 48, integer_part, sign);
